...
class ObjaverseDataset(Dataset):
    ...
    def __init__(self, opt: Options, training=True):
        ...
        # TODO: carregue a lista de objetos para treinamento
        self.items = []
    ...
    def __getitem__(self, idx):
        ...
        # TODO: escolha visualizações com base nas configurações de renderização
        if self.training:
        ...
        for vid in vids:
            ...
            try:
                # TODO: carregar dados (modify self.client here)
        ...
        # feitos de câmera normalizados como no papel (transformar a primeira pose em uma posição fixa)
        ...
        # data augmentation
        if self.training:
            # aplique distorção de grade aleatória para simular inconsistência 3D
            if random.random() < self.opt.prob_grid_distortion:
                images_input[1:] = grid_distortion(images_input[1:])
            # aplique tremulação da câmera (apenas para entrada!)
            if random.random() < self.opt.prob_cam_jitter:
                cam_poses_input[1:] = orbit_camera_jitter(cam_poses_input[1:])

        images_input = TF.normalize(images_input, IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)

        # redimensionar e renderizar imagens reais, intervalo ainda em [0, 1]
